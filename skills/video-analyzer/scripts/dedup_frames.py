...
ГРАНИЦА ПРИМЕНИМОСТИ (важно): эта метрика смотрит на ВЕСЬ кадр. В записях «слайд + камера
спикера» она НЕ схлопывает надёжно кадры одного слайда с разным положением говорящего:
крупная подвижная плашка спикера меняет сигнатуру не меньше, чем смена самого слайда. Ни
порогом, ни обрезкой, ни краями/локальностью изменений это чисто не отделяется от двух разных
слайдов на одном тёмном шаблоне (проверено на реальной записи). Поэтому «тот же слайд —
разная голова» дедупит СКРИНЕР (шаг 5b SKILL.md, зрение-LLM читает содержимое слайда), а не
этот скрипт. Тут держим порог консервативным: лучше оставить лишнее, чем потерять слайд.
